...
print("Training with Adam (lr=0.001, batch_size=64)...")
num_epochs = 30
train_losses = []
val_losses = []
train_accuracies = []
...
for epoch in range(num_epochs):
    ...
    if val_acc > best_val_acc:
        ...
        best_epoch = epoch
        torch.save(model.state_dict(), os.path.join(script_dir, 'model_exp2_adam.pth'))
...
np.save(os.path.join(script_dir, 'exp2_results.npy'), results, allow_pickle=True)
print("Results saved to exp2_results.npy")
...
model.load_state_dict(torch.load(os.path.join(script_dir, 'model_exp2_adam.pth')))
model.eval()
...
submission.to_csv(os.path.join(script_dir, 'submission_exp2_adam.csv'), index=False)
